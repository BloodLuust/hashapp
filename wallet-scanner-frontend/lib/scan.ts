...
export type StartModeRequest = {
  mode: 'random' | 'range' | 'specific'
  min_hex?: string
  max_hex?: string
  randomize?: boolean
  input?: string
...
export async function startModeScan(body: StartModeRequest): Promise<{ jobs: { id: string; chain: string }[] }>{
  const r = await fetch('/api/scan/start', {
    method: 'POST',
    headers: { 'content-type': 'application/json', accept: 'application/json' },
    body: JSON.stringify(body),
  })
  if (!r.ok) throw new Error(`Failed to start mode scan: ${r.status}`)
  return r.json()
}
